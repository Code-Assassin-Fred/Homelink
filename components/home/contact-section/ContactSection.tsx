...
const contactDetails = [
  {
    title: "Email",
    value: "info@homelinkafrica.com",
    href: "mailto:info@homelinkafrica.com",
    icon: (
  ...
  return (
    <section id="contact" className="scroll-mt-24 bg-offwhite py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading
          title="Contact Us"
          subtitle="Tell us what you need and we will help you take the next step with clarity and care."
        />
          ...
          <div className="space-y-6">
              ...
              <ul className="space-y-5">
                {contactDetails.map((item) => (
                  <li key={item.title} className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-full bg-jade/20 flex items-center justify-center shrink-0 text-jade">
                      {item.icon}
                    </div>
                    <div>
                      <p className="font-heading font-bold text-sm text-lime mb-1">
                        {item.title}
                      </p>
                      {item.href ? (
                        <a href={item.href} className="text-white text-sm hover:text-lime transition-colors duration-200">
                          {item.value}
                        </a>
                      ) : (
            ...
            <div className="bg-white rounded-[32px] border border-forest/10 shadow-sm p-8 sm:p-10">
              ...
              <p className="text-sm text-charcoal mt-4">
                We support diaspora clients across multiple time zones with clear, dependable communication.
              </p>
            </div>
          </div>
        </div>
